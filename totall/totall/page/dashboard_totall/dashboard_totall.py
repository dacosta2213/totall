...
@frappe.whitelist()
def get_clientes(doctype, timespan, field, start=0):
	...
	if doctype == "Customer":
		...
		items = get_productos(doctype, filters, [], field)
...
@frappe.whitelist()
def get_productos(doctype, filters, items, field, start=0, limit=20):
	"""return all items"""
	...
	for val in x:
		...
		data = frappe.db.sql('''select item_code from `tabSales Invoice Item` where item_code = %s''', (val.name), as_list=1)
		masvendidos = destructure_tuple_of_tuples(data)
