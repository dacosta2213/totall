...
frappe.provide("retail.pov");

// tuve que cambiar los templates que estan en erpnext/public/js/payments por pendejo (no supe como integrarlo en esta mierda)


frappe.pages['pov'].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __('Punto de Venta'),
		single_column: true
	});
	...
	frappe.db.get_value('POS Settings', {name: 'POS Settings'}, 'is_online', (r) => {
		if (r && r.use_pos_in_offline_mode && cint(r.use_pos_in_offline_mode)) {
			// offline
			wrapper.pos = new retail.pov.PointOfSale(wrapper);
			cur_pos = wrapper.pos;
		} else {
			// online
...
frappe.pages['pov'].refresh = function (wrapper) {
...
retail.pov.PointOfSale = erpnext.taxes_and_totals.extend({
	...
	get_data_from_server: function (callback) {
		var me = this;
		frappe.call({
			method: "retail.retail.page.pov.pov.get_pos_data",
			freeze: true,
	...
	make_control: function() {
		...
		this.wrapper.html(frappe.render_template("pov", this.frm.doc));
		this.make_search();
		this.make_customer();
	...
	render_list_customers: function () {
		...
		if(this.si_docs.length) {
			this.si_docs.forEach(function (data, i) {
				for (var key in data) {
					html += frappe.render_template("pov_invoice_list", {
						sr: i + 1,
						name: key,
	...
	make_customer: function () {
		...
		if(!this.party_field) {
			if(this.page.wrapper.find('.pos-bill-toolbar').length === 0) {
				$(frappe.render_template('pov_customer_toolbar', {
		...
		if (this.items.length > 0) {
			$.each(this.items, function(index, obj) {
				if(index < me.page_len) {
					$(frappe.render_template("pov_item", {
						item_code: obj.name,
	...
	render_selected_item: function() {
		...
		if(this.child_doc.length) {
			...
			this.selected_row = $(frappe.render_template("pov_selected_item", this.child_doc[0]))
	...
	refresh: function (update_paid_amount) {
		...
		this.set_primary_action();
				...
				$('.ayudachico').hide();
				$('.ayudagrande').hide();
				$(".pos-bill-toolbar").prepend('<button data-video-id="CwnT8lyunBI"  class="js-modal-btn ayudachico"><i class="far fa-question-circle"></i></button>')
				$(".pos-bill-toolbar").append('<button data-video-id="ofuiUPp1pHM"  class="js-modal-btn ayudachico"><i class="far fa-question-circle"></i></button>')

				$('.page-actions').prepend('<button data-video-id="9M4MDRQJz_g"  class="btn-xs js-modal-btn ayudagrande "><i class="far fa-question-circle"></i>  Ayuda</button>');
	...
	show_items_in_item_cart: function () {
		...
		$.each(this.frm.doc.items || [], function (i, d) {
			$(frappe.render_template("pov_bill_item_new", {
	...
	set_taxes: function () {
		...
		$.each(taxes, function (i, d) {
			if (d.tax_amount && cint(d.included_in_print_rate) == 0) {
				$(frappe.render_template("pov_tax_row", {
	...
	set_primary_action: function () {
		var me = this;
		this.page.set_primary_action(__("Nueva Venta"), function () {
			me.make_new_cart()
			me.make_menu_list()
		}, "fa fa-plus")
	...
	sync_sales_invoice: function () {
		...
		if ((this.si_docs.length || this.email_queue_list || this.customers_list) && !this.freeze) {
			...
			frappe.call({
				method: "retail.retail.page.pov.pov.make_invoice",
